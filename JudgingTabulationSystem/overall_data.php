<?php  
  ...
  if (isset($_POST["title"])) {
    ...
    elseif ($_POST["title"] != '') {
      ...
      $query5 = "SELECT *, SUM(Score) AS TotalScore FROM tabulate 
                JOIN contestant ON contestant.ContestantID = tabulate.ContestantID 
                GROUP BY ContestantName 
      ...
      while ($row5 = mysqli_fetch_array($result5)) {
          $count++;
          if ($rank == 0 || $last_score != $row5['TotalScore']) {
              $last_score = $row5['TotalScore'];
              $rank = $count;
          }

          echo "<tr><td>#".$row5['ContestantID']." ".$row5['ContestantName']."</td>
          <td>".$row5['TotalScore']."</td>
          <td>".$rank."</td></tr>";
      }
