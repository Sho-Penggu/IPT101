			...
			<h3 class="page-header">Records <span class="animated fadeInDown right-loged"><a href="report-admin.php"><input type="button" class="report" title="Report"></a> <a href="index.php?logout=1"><input type="button" class="logout" title="Logout"></a></span></h3>
				...
				<div class="tab-content responsive">
					...
					<div class="tab-pane fade table-responsive" id="four">
						...
						<!-- Add the ComboBox (dropdown select) -->
						<select id="categorySelect" class="form-control">
						    <option value="">Select a Category</option>
						    <?php
						        // Fetch categories from the database
						        $categoryQuery = "SELECT * FROM category";
						        $categoryResult = mysqli_query($dbcon, $categoryQuery);
						...
						<table class="table table-hover table-striped">
							...
							<tbody>
								<?php
									include('config.php');
									$query = "SELECT * FROM contestantcategory JOIN contestant ON contestant..CategoryID = contestantcategory.CategoryID ORDER BY TitleID, ContestantName, CategoryName ASC ";
									$result = mysqli_query($dbcon, $query);
									while ($row = mysqli_fetch_array($result)) {
									ContestantID = contestantcategory.ContestantID JOIN category ON category	echo "<tr>";
...
<!--Script for Filtering data in ContestantCategory -->
<script>
    $(document).ready(function(){
        // Function to filter and display data based on selected category
        $('#categorySelect').change(function(){
            var selectedCategoryID = $(this).val();

            // AJAX call to fetch and display filtered data
            $.ajax({
                type: 'POST',
                url: 'filter_data.php', // Replace with the PHP file handling the filtering
                data: { categoryID: selectedCategoryID },
                success: function(response) {
                    $('#four tbody').html(response);
                }
            });
        });
    });
